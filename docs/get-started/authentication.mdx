...
| OpenAI-compatible endpoint user                                        | [Use an OpenAI-compatible API](#openai)                          | No                                                          |
| [Headless mode](#headless)                                             | [Use Gemini API Key](#gemini-api), [Vertex AI](#vertex-ai), or [Use an OpenAI-compatible API](#openai) | No (for Gemini API Key and OpenAI-compatible API)<br /> [Yes](#set-gcp) (for Vertex AI) |
...
## Use an OpenAI-compatible API <a id="openai"></a>

If you have an OpenAI-compatible endpoint, you can use it with Gemini CLI
instead of a Google account or a Gemini API key. This covers local servers such
as Ollama and LM Studio, self-hosted gateways, and hosted providers. Streaming
responses, tool calling, images, and token usage reporting work against any
endpoint that implements the OpenAI Chat Completions API. Audio, video, and file
references have no equivalent in that API and aren't sent to the endpoint.

To authenticate and use Gemini CLI with an OpenAI-compatible API:

1. Set the environment variables that describe your endpoint.

   | Variable                 | Required | Description                                                                                                                              |
   | :----------------------- | :------- | :--------------------------------------------------------------------------------------------------------------------------------------- |
   | `GEMINI_API_TYPE`        | Yes      | Enables this mode. The value is case-sensitive and must be exactly `openai`.                                                             |
   | `GEMINI_OPENAI_BASE_URL` | Yes      | Base URL of the endpoint. If the URL has no path, Gemini CLI appends `/v1`; otherwise the path is used as given.                        |
   | `GEMINI_OPENAI_API_KEY`  | No       | Bearer token sent in the `Authorization` header. Omit it if your endpoint doesn't require authentication, which is typical for local servers. |
   | `GEMINI_OPENAI_MODELID`  | Yes      | Identifier of the model to use, in the format your endpoint expects (for example, `gpt-4o`).                                             |
   | `GEMINI_OPENAI_HEADERS`  | No       | Extra request headers as a JSON object. These replace the built-in headers, including `Authorization`.                                   |
   ...
   <Tabs>
     ...
     ```bash
     # Replace the placeholder values with your endpoint's details
     ...
     ```powershell
     # Replace the placeholder values with your endpoint's details
     $env:GEMINI_API_TYPE="openai"
     $env:GEMINI_OPENAI_BASE_URL="https://api.example.com/v1"
     $env:GEMINI_OPENAI_API_KEY="YOUR_API_KEY"
...
2. Start the CLI:
   ...
   Gemini CLI selects **OpenAI API (compatible)** automatically. You can also
   choose it from the authentication dialog, which then saves it as your
   selected authentication type.

{/* prettier-ignore */}
> [!NOTE]
> `GEMINI_OPENAI_MODELID` is authoritative in this mode. The `--model` flag and
> the `GEMINI_MODEL` variable don't apply, because Gemini CLI's model aliases
> and defaults refer to Gemini models.

{/* prettier-ignore */}
> [!NOTE]
> `GEMINI_OPENAI_HEADERS` takes a flat JSON object of header names to values. Use
> it when your endpoint authenticates with something other than a bearer token,
> such as Azure's `api-key`, or when it requires a tenant identifier:
>
> ```bash
> export GEMINI_OPENAI_HEADERS='{"api-key":"YOUR_KEY","X-Tenant":"acme"}'
> ```
>
> Configured headers replace the built-in ones, so a value here overrides
> `Authorization` and `Content-Type` as well. Header names are case-insensitive,
> so `authorization` replaces the built-in `Authorization` rather than being sent
> alongside it; writing the same header twice under different names is an error.
>
> Invalid JSON, an invalid header name, a value that isn't a string, number, or
> boolean, a C0 control character other than tab, a DEL, and a character above
> U+00FF are all reported at startup instead of being ignored. Header values are
> bytes, so encode anything outside Latin-1 first.

{/* prettier-ignore */}
> [!IMPORTANT]
> Setting `GEMINI_API_TYPE=openai` overrides the authentication type saved in
> `security.auth.selectedType`. An administrator's `security.auth.enforcedType`
> setting still takes precedence.

{/* prettier-ignore */}
> [!WARNING]
> Treat `GEMINI_OPENAI_API_KEY` as a sensitive credential, the same as any other
> API key. Use it only with an endpoint you trust, because every prompt and
> every tool result is sent to that endpoint, including file contents that
> Gemini CLI reads on your behalf.
...
- [Vertex AI](#vertex-ai)
- [Use an OpenAI-compatible API](#openai)
